...
const INTEGRATIONS: Integration[] = [
  ...
  // Monitoring
  {
    ...
    description:
      "Open-source enterprise network monitoring and alerting platform.",
  },
  {
    id: "prometheus",
    name: "Prometheus",
...
function IntegrationCard({ integration }: { integration: Integration }) {
  ...
  const [zabbixConn, setZabbixConn] = useState<ZabbixConnectionState>({
    connected:
      integration.id === "zabbix" && integration.status === "connected",
    hostCount: 0,
    activeAlerts: 0,
  });
  ...
  function handleZabbixConnectionChange(state: ZabbixConnectionState) {
    setZabbixConn(state);
    if (state.connected) {
      toast.success("Zabbix connected");
    }
  }
    ...
    <>
      ...
      >
        <CardContent className="pt-4 pb-4 space-y-3">
          <div className="flex items-start justify-between gap-3">
            <div className="flex items-center gap-3">
              <div className="relative shrink-0">
                ...
                {/* Connection status dot — only on Zabbix card */}
                {isZabbix && (
                  <span
                    className={cn(
              ...
              <div className="min-w-0">
                ...
                {isZabbix ? (
                  <p className="text-xs text-muted-foreground">
                    {effectiveConnected
                      ? `Connected to ${zabbixServerShort}`
                      : "Not configured"}
                  </p>
                ) : (
                  effectiveConnected &&
                ...
                {isZabbix && effectiveConnected && zabbixConn.lastSync && (
                  <p className="text-xs text-muted-foreground">
                    Synced {formatRelativeTime(zabbixConn.lastSync)}
                  </p>
                )}
              </div>
            </div>
          ...
          {/* Zabbix stats row when connected */}
          {isZabbix && effectiveConnected && (
            <div className="flex items-center gap-3 rounded-lg border border-red-500/20 bg-red-500/5 px-3 py-1.5">
              <span className="text-xs text-muted-foreground">
                <span className="font-medium text-foreground">
                  {zabbixConn.hostCount}
                </span>{" "}
                hosts
              </span>
              ...
              <span className="text-xs text-muted-foreground">
                <span
                  className={cn(
                    "font-medium",
                    zabbixConn.activeAlerts > 0
                      ? "text-rose-400"
                      : "text-foreground",
                  )}
                >
                  {zabbixConn.activeAlerts}
                </span>{" "}
                active alerts
              </span>
            </div>
                  ...
                  <>
                    ...
                    <Button
                      ...
                      onClick={() => {
                        ...
                        toast.success("Zabbix sync triggered");
                      }}
            ...
            <div className="pt-2 border-t border-border space-y-3">
              <div className="space-y-1.5">
                ...
                <Input
                  defaultValue={
                    connected
                      ? `https://${integration.id}.fibernms.internal`
                      : ""
                  }
                  placeholder="https://…"
