// ─── Super Admin Mock Data (Seed-42 Deterministic PRNG) ────────────────────────
import type {
  AdminUser,
  AuditEvent,
  Company,
...
// Re-export type aliases needed by existing pages
export type {
  AlertSeverity,
  AuditSeverity,
  CompanyStatus,
...
function randDate(daysAgoMin: number, daysAgoMax: number): string {
  const ms = Date.now() - randInt(daysAgoMin, daysAgoMax) * 86_400_000;
...
function randPastDate(
  ...
  const ms = baseMs + randInt(daysAgoMin, daysAgoMax) * 86_400_000;
  return new Date(Math.min(ms, Date.now())).toISOString();
}
function fmtId(prefix: string, n: number, pad = 4): string {
  return `${prefix}-${String(n).padStart(pad, "0")}`;
}
function randIp(): string {
  return `${randInt(10, 220)}.${randInt(0, 255)}.${randInt(0, 255)}.${randInt(1, 254)}`;
}
function randPhone(region: string): string {
  if (region === "India") return `+91-${randInt(7000000000, 9999999999)}`;
  if (region === "US") return `+1-${randInt(2000000000, 9999999999)}`;
  if (region === "EU") return `+44-${randInt(100000000, 999999999)}`;
  if (region === "APAC") return `+81-${randInt(1000000000, 9999999999)}`;
  return `+971-${randInt(500000000, 599999999)}`;
}

// ─── Constants ────────────────────────────────────────────────────────────────
const PLAN_MONTHLY: Record<CompanyPlan, number> = {
  BASIC: 200,
  PROFESSIONAL: 500,
  ENTERPRISE: 2000,
  ULTRA: 5000,
};
const PLAN_YEARLY: Record<CompanyPlan, number> = {
  BASIC: 2000,
  PROFESSIONAL: 5000,
  ENTERPRISE: 20000,
  ULTRA: 50000,
};
const PLAN_DEVICE_LIMIT: Record<CompanyPlan, number> = {
  BASIC: 1_000,
  PROFESSIONAL: 10_000,
  ENTERPRISE: 100_000,
  ULTRA: 1_000_000,
};
const PLAN_API_LIMIT: Record<CompanyPlan, number> = {
  BASIC: 100_000,
  PROFESSIONAL: 1_000_000,
  ENTERPRISE: 10_000_000,
  ULTRA: 100_000_000,
};
const PLAN_DATA_LIMIT: Record<CompanyPlan, number> = {
  BASIC: 50,
  PROFESSIONAL: 500,
  ENTERPRISE: 5000,
  ULTRA: 50000,
};
const PLAN_ALERT_LIMIT: Record<CompanyPlan, number> = {
  BASIC: 10_000,
  PROFESSIONAL: 100_000,
  ENTERPRISE: 1_000_000,
  ULTRA: 10_000_000,
};
const TAX_BY_REGION: Record<
  CompanyRegion,
  { rate: number; type: "GST" | "VAT" | "NONE" }
> = {
...
const COMPANY_NAMES: Record<CompanyRegion, string[]> = {
  India: [
...
// Plan distribution: BASIC(12), PRO(18), ENTERPRISE(15), ULTRA(9)
const PLAN_POOL: CompanyPlan[] = [
  ...Array(12).fill("BASIC" as CompanyPlan),
  ...Array(18).fill("PROFESSIONAL" as CompanyPlan),
  ...Array(15).fill("ENTERPRISE" as CompanyPlan),
  ...Array(9).fill("ULTRA" as CompanyPlan),
];
// Status distribution: Active(35), Trial(10), Suspended(6), Expired(3)
const STATUS_POOL = [
...
export const mockCompanies: Company[] = REGION_DATA.flatMap(
  ({ region, countries, count }) => {
    ...
    return names.map((name) => {
      ...
      const domainBase = name
        .toLowerCase()
        .replace(/[^a-z0-9]/g, "")
        .slice(0, 14);
      const tlds: Record<CompanyRegion, string> = {
        India: ".in",
        EU: ".eu",
        APAC: ".net",
        MENA: ".ae",
      ...
      if (status === "trial") usagePct = randInt(5, 20) / 100;
        ...
        const bucket = rand();
        if (bucket < 0.2)
          usagePct = randInt(70, 95) / 100; // warning zone
        else if (bucket < 0.1)
          usagePct = randInt(90, 100) / 100; // critical
        ...
        plan === "BASIC"
          ...
          : plan === "PROFESSIONAL"
            ...
            : plan === "ENTERPRISE"
              ...
              : randInt(50, 500);
      const mrr =
        status === "active" || status === "trial" ? PLAN_MONTHLY[plan] : 0;
      ...
      return {
        ...
        alertsLimit,
        activeUsers,
        totalAlerts: randInt(100, 15000),
        mrr,
        lastActiveAt,
        isSoftDeleted: false,
        ...(status === "trial"
          ? { trialEndsAt: randPastDate(createdAt, 14, 30) }
          : {}),
        // compat aliases
        onboardedAt: createdAt,
        monthlyRevenue: mrr,
        alertsGenerated: randInt(100, 15000),
...
const ROLE_WEIGHTS: Array<{ role: AdminUser["role"]; weight: number }> = [
...
function pickRole(): AdminUser["role"] {
  const r = rand();
  let cum = 0;
...
export const mockUsers: AdminUser[] = mockCompanies.flatMap((company, ci) => {
  const count = randInt(18, 26);
  return Array.from({ length: count }, (): AdminUser => {
    ...
    const role = pickRole();
    const status: AdminUser["status"] = rand() > 0.15 ? "active" : "disabled";
...
export const mockOrders: Order[] = Array.from(
  ...
  (_, i): Order => {
    ...
    const cycle: Order["billingCycle"] = rand() > 0.3 ? "monthly" : "yearly";
    const price =
      cycle === "yearly"
        ? PLAN_YEARLY[company.plan]
        : PLAN_MONTHLY[company.plan];
    const status = pick(ORDER_STATUS_POOL);
    const createdAt = randDate(10, 730);
    const dueDateMs = new Date(createdAt).getTime() + 30 * 86_400_000;
    const dueDate = new Date(dueDateMs).toISOString();
    const paidAt =
      status === "paid" ? randPastDate(createdAt, 0, 5) : undefined;
    const nextRenewal =
      status === "paid"
        ? new Date(
            new Date(createdAt).getTime() +
              (cycle === "yearly" ? 365 : 30) * 86_400_000,
          ).toISOString()
        : undefined;
    return {
      id: fmtId("ord", i + 1, 5),
...
export const mockInvoices: Invoice[] = mockOrders.map((order, i): Invoice => {
  const company = mockCompanies.find((c) => c.id === order.companyId);
  const region = (company?.region ?? "US") as CompanyRegion;
...
const AUDIT_TEMPLATES: Array<{
  ...
  details: (c: Company, u: AdminUser | null) => string;
}> = [
  {
    action: "user.created",
    category: "user",
  ...
  },
  {
    ...
    targetType: "Session",
    severity: "info",
    details: (_c, u) => `User ${u?.name ?? "Unknown"} logged in successfully`,
  },
  {
    ...
    severity: "warning",
    details: (_c, u) =>
      `Failed login attempt for ${u?.email ?? "unknown user"}`,
  },
  {
    ...
    targetType: "User",
    severity: "warning",
    details: (_c, u) => `User account ${u?.email ?? "unknown"} was disabled`,
  },
  {
  ...
  },
  {
    ...
    severity: "critical",
    details: (_c, u) =>
      `Active sessions force-terminated for ${u?.email ?? "user"}`,
  },
  {
    action: "user.invited",
    category: "user",
  ...
  },
  {
    ...
    severity: "info",
    details: () =>
      `Bulk import of ${randInt(5, 50)} users completed successfully`,
  },
  {
  ...
  },
  {
    ...
    severity: "info",
    details: (c) =>
      `Plan upgraded from PROFESSIONAL to ENTERPRISE for ${c.name}`,
  },
  {
    ...
    severity: "warning",
    details: (c) =>
      `Plan downgraded from ENTERPRISE to PROFESSIONAL for ${c.name}`,
  },
  {
    action: "device.added",
    category: "device",
  ...
  },
  {
    ...
    targetType: "User",
    severity: "warning",
    details: (_c, u) => `Password changed for ${u?.email ?? "user"}`,
  },
  {
    ...
    targetType: "User",
    severity: "info",
    details: (_c, u) => `MFA enabled for ${u?.email ?? "user"}`,
  },
  {
  ...
  (_, i): AuditEvent => {
    ...
    const user =
      usersForCompany.length > 0
        ? usersForCompany[i % usersForCompany.length]
        : null;
    const tmpl = pick(AUDIT_TEMPLATES);
    return {
      id: fmtId("aud", i + 1, 6),
      userId: user?.id ?? fmtId("usr", i + 1, 5),
      userEmail: user?.email ?? "system@fibernms.internal",
      companyId: company.id,
      companyName: company.name,
      action: tmpl.action,
      targetType: tmpl.targetType,
      targetId: fmtId("tgt", i + 1, 5),
      targetName: user?.name ?? company.name,
...
export const mockSecurityEvents: SecurityEvent[] = Array.from(
  { length: 250 },
  (_, i): SecurityEvent => {
    const company = mockCompanies[i % mockCompanies.length];
    const usersForCompany = mockUsers.filter((u) => u.companyId === company.id);
    ...
    const secSeverity = (
      eventType === "mfa_bypass_attempt"
        ...
        : eventType === "suspicious_activity"
          ? "high"
          : eventType === "rate_limit_exceeded" || eventType === "ip_blocked"
            ? "medium"
            : "low"
...
// ─── IP Tracking ───────────────────────────────────────────────────────────────
export const mockIPTracking: IPTrackingEntry[] = Array.from(
  { length: 60 },
  (_, i): IPTrackingEntry => {
    const company = mockCompanies[i % mockCompanies.length];
    const usersForCompany = mockUsers.filter((u) => u.companyId === company.id);
...
export const mockSuspiciousActivity: SuspiciousActivity[] = Array.from(
  { length: 20 },
  (_, i): SuspiciousActivity => {
    const user = mockUsers[randInt(0, mockUsers.length - 1)];
    const sev = pick([
      "critical",
      "high",
      "medium",
    ] as SuspiciousActivity["severity"][]);
    return {
      id: fmtId("susp", i + 1, 4),
      ipAddress: randIp(),
      userEmail: rand() > 0.4 ? user.email : undefined,
...
// ─── Login Attempts Time Series (30 days hourly) ───────────────────────────────
export const mockLoginAttempts: LoginAttempt[] = Array.from(
  { length: 30 * 24 },
  (_, i): LoginAttempt => {
...
// ─── MFA Status ────────────────────────────────────────────────────────────────
export const mockMFAStatus: MFAStatus[] = mockUsers.map(
  (u): MFAStatus => ({
    userId: u.id,
    userEmail: u.email,
    companyName: u.companyName,
    mfaEnabled: u.mfaEnabled,
    lastMFAEvent: u.mfaEnabled ? randDate(0, 30) : undefined,
    backupCodesAvailable: u.mfaEnabled && rand() > 0.3,
  }),
);

// ─── Usage Metrics ─────────────────────────────────────────────────────────────
export const mockUsageMetrics: UsageMetric[] = mockCompanies.map(
  (c): UsageMetric => ({
    companyId: c.id,
    companyName: c.name,
    plan: c.plan,
...
export const mockCompanyActivities: CompanyActivity[] = Array.from(
  { length: 350 },
  (_, i): CompanyActivity => {
    const company = mockCompanies[i % mockCompanies.length];
    const usersForCompany = mockUsers.filter((u) => u.companyId === company.id);
    const actor =
      usersForCompany.length > 0
        ? usersForCompany[i % usersForCompany.length].email
        : "system@fibernms.internal";
    return {
      id: fmtId("act", i + 1, 5),
      companyId: company.id,
...
const ALERT_TEMPLATES: Array<{
...
}> = [
  {
    ...
    message:
      "Complete loss of API gateway connectivity affecting India region tenants",
    severity: "critical",
    category: "outage",
  },
  {
  ...
  },
  {
    ...
    message:
      "Payment processing API returning timeouts; affecting invoice generation",
    severity: "critical",
    category: "api",
  },
  {
  ...
  },
  {
    ...
    message:
      "Real-time event relay experiencing packet loss; NOC dashboards may lag",
    severity: "medium",
    category: "resource",
  },
  {
  ...
  },
  {
    ...
    message:
      "Outbound webhook delivery queue backing up; events delayed 15min+",
    severity: "critical",
    category: "api",
  },
  {
  ...
  },
  {
    ...
    message:
      "Entire MENA API cluster offline following datacenter network failure",
    severity: "critical",
    category: "outage",
  },
  {
  ...
  },
  {
    ...
    message:
      "Cache cluster hit 95% memory; mass eviction causing DB read storms",
    severity: "critical",
    category: "resource",
  },
  {
  ...
  },
  {
    ...
    message:
      "Payment webhook verification failing; billing events not processing",
    severity: "critical",
    category: "api",
  },
  {
    ...
    message:
      "Auto-discovery daemon in crash loop; new devices not being registered",
    severity: "critical",
    category: "outage",
  },
];

export const mockSystemAlerts: SystemAlert[] = ALERT_TEMPLATES.map(
  (tmpl, i): SystemAlert => {
    const timestamp = randDate(0, 60);
    const isResolved = rand() > 0.45;
    const resolvedAt = isResolved
      ? new Date(
          new Date(timestamp).getTime() + randInt(30, 480) * 60000,
        ).toISOString()
      : undefined;
    return {
      id: fmtId("alrt", i + 1, 4),
...
export const platformStats: PlatformStats = {
  ...
  suspendedCompanies: mockCompanies.filter((c) => c.status === "suspended")
    .length,
  totalUsers: mockUsers.length,
  activeSubscriptions: mockCompanies.filter((c) => c.status === "active")
    .length,
...
export function getUsersByCompany(companyId: string): AdminUser[] {
...
export function getAuditByCompany(companyId: string): AuditEvent[] {
  return mockAuditEvents.filter((e) => e.companyId === companyId);
}
export function getActivitiesByCompany(companyId: string): CompanyActivity[] {
  return mockCompanyActivities.filter((a) => a.companyId === companyId);
}
export function getUsageByCompany(companyId: string): UsageMetric | undefined {
  return mockUsageMetrics.find((m) => m.companyId === companyId);
}

// ─── Legacy compat aliases (used by existing super-admin pages) ────────────────
export const MOCK_COMPANIES = mockCompanies;
export const MOCK_ADMIN_USERS = mockUsers;
export const MOCK_ORDERS = mockOrders;
export const MOCK_INVOICES = mockInvoices;
export const MOCK_AUDIT_EVENTS = mockAuditEvents;
export const MOCK_SECURITY_EVENTS = mockSecurityEvents;
export const MOCK_SYSTEM_ALERTS = mockSystemAlerts;

export function getCompaniesByRegion(region: string): Company[] {
  return mockCompanies.filter((c) => c.region === region);
}
export function getCompaniesByPlan(plan: CompanyPlan): Company[] {
  return mockCompanies.filter((c) => c.plan === plan);
}
export function getAuditEventsByCompany(companyId: string): AuditEvent[] {
  return getAuditByCompany(companyId);
}
export function getActiveSystemAlerts(): SystemAlert[] {
  return mockSystemAlerts.filter((a) => !a.isResolved);
}
...
export const REVENUE_METRICS: RevenueMetrics = (() => {
  ...
  const revenueByPlan: Record<CompanyPlan, number> = {
    BASIC: 0,
    PROFESSIONAL: 0,
    ENTERPRISE: 0,
    ULTRA: 0,
  ...
  return {
    ...
    activeSubscriptions: mockCompanies.filter((c) => c.status === "active")
      .length,
    trialSubscriptions: mockCompanies.filter((c) => c.status === "trial")
      .length,
    suspendedSubscriptions: mockCompanies.filter(
      (c) => c.status === "suspended",
    ).length,
    expiredSubscriptions: mockCompanies.filter((c) => c.status === "expired")
      .length,
    revenueByPlan,
    revenueByRegion,
  };
