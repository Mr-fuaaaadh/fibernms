import { o } from "./index-DluTrB5k.js";
...
const useFormControlContext = () => o.useContext(HookFormControlContext);
...
function useFormState(props) {
  ...
  const [formState, updateFormState] = o.useState(control._formState);
  const _localProxyFormState = o.useRef({
    isDirty: false,
    isLoading: false,
    dirtyFields: false,
    touchedFields: false,
  ...
  return o.useMemo(() => getProxyFormState(formState, control, _localProxyFormState.current, false), [formState, control]);
}
...
function useWatch(props) {
  ...
  const _defaultValue = o.useRef(defaultValue);
  const _compute = o.useRef(compute);
  const _computeFormValues = o.useRef(void 0);
  const _prevControl = o.useRef(control);
  const _prevName = o.useRef(name);
  _compute.current = compute;
  const [value, updateValue] = o.useState(() => {
  ...
  const getCurrentOutput = o.useCallback((values) => {
    const formValues = generateWatchOutput(name, control._names, values || control._formValues, false, _defaultValue.current);
    return _compute.current ? _compute.current(formValues) : formValues;
  }, [control._formValues, control._names, name]);
  const refreshValue = o.useCallback((values) => {
    if (!disabled) {
      const formValues = generateWatchOutput(name, control._names, values || control._formValues, false, _defaultValue.current);
      if (_compute.current) {
  ...
  o.useEffect(() => control._removeUnmounted());
  const controlChanged = _prevControl.current !== control;
  const prevName = _prevName.current;
  const computedOutput = o.useMemo(() => {
    if (disabled) {
      return null;
    }
...
function useController(props) {
  ...
  const defaultValueMemo = o.useMemo(() => get(control._formValues, name, get(control._defaultValues, name, defaultValue)), [control, name, defaultValue]);
  ...
  const _props = o.useRef(props);
  const _previousNameRef = o.useRef(void 0);
  const _registerProps = o.useRef(control.register(name, {
    ...props.rules,
    value,
    ...isBoolean(props.disabled) ? { disabled: props.disabled } : {}
  }));
  _props.current = props;
  const fieldState = o.useMemo(() => Object.defineProperties({}, {
    invalid: {
      enumerable: true,
  ...
  const onChange = o.useCallback((event) => _registerProps.current.onChange({
    target: {
  ...
  const onBlur = o.useCallback(() => _registerProps.current.onBlur({
    target: {
